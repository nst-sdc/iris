...
import { setupLoadingListeners } from "../hooks/use-loading";
...
export interface LoadingScreenProps {
  /**
   * Minimum duration in milliseconds that the loading screen should be shown
   * This ensures smooth transitions even for fast page loads
   */
  ...
  /**
   * Whether to show the CPU architecture animation
   */
  showCpuAnimation?: boolean;
...
export default function LoadingScreen({
  ...
  const [loadingPhrase, setLoadingPhrase] = useState(loadingText);
  
  // Loading phrases for a futuristic feel
  ...
  // Effect to handle route changes and global loading events
  useEffect(() => {
    ...
    const startLoadingAnimation = (options?: { minimumDuration?: number }) => {
      const duration = options?.minimumDuration || minimumDuration;
      setIsLoading(true);
      setProgress(0);
      ...
      const timeout = setTimeout(() => {
        ...
        // Keep loading screen visible for a short time after reaching 100%
        setTimeout(() => {
          setIsLoading(false);
        }, 300);
      }, duration);
      
      return { interval, progressInterval, timeout };
    };
    ...
    // Start initial loading animation for route change
    const intervals = startLoadingAnimation();
    
    // Setup global loading event listeners
    const cleanup = setupLoadingListeners(startLoadingAnimation, endLoadingAnimation);
    
    return () => {
      // Clean up all intervals and timeouts
      if (intervals.interval) clearInterval(intervals.interval);
      if (intervals.progressInterval) clearInterval(intervals.progressInterval);
      if (intervals.timeout) clearTimeout(intervals.timeout);
      cleanup();
    };
  }, [pathname, searchParams, minimumDuration, loadingPhrases]);
    ...
    <AnimatePresence mode="wait">
      {isLoading && (
        ...
        >
          <div className="relative w-full max-w-md px-4">
            ...
            >
              <h2 className="text-xl font-medium mb-2">
                <GlitchText text={loadingPhrase} glitchIntensity={4} glitchFrequency={1500} />
              </h2>
            ...
            {showProgressBar && (
              <motion.div
                ...
                className="w-full bg-dark-300 rounded-full h-1.5 mb-4 overflow-hidden"
              >
                <motion.div
            ...
            <div className="absolute -z-10 inset-0 overflow-hidden">
              <div className="absolute top-1/2 left-0 w-16 h-16 -translate-y-1/2 -translate-x-1/2 rounded-full bg-primary/5 blur-xl"></div>
              <div className="absolute top-1/3 right-0 w-24 h-24 -translate-y-1/2 translate-x-1/2 rounded-full bg-secondary/5 blur-xl"></div>
            </div>
          </div>
          ...
          <div className="absolute inset-0 -z-10 overflow-hidden opacity-20">
            <svg width="100%" height="100%" className="absolute inset-0">
              <pattern id="circuit-pattern" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
                ...
                <path d="M 100 50 L 50 50 L 50 100" fill="none" stroke="currentColor" strokeWidth="0.5" />
          ...
          {/* Noise Overlay */}
          <NoiseOverlay opacity={0.03} blendMode="soft-light" zIndex={45} />
        </motion.div>
      )}
    </AnimatePresence>
  );
